...
//
// In the insertion method you specifiy a price and an ID. So the ID is used as
// the key but insertion in tree is determined by price.
//
//
// BokkyPooBah's Red-Black Tree Library v1.0-pre-release-a - MODIFIED
//
...
library BokkyPooBahsRedBlackTreeLibrary {
    ...
    function getBestKey(Tree storage self,uint _min) internal view returns (uint _key){

        _key = self.root;
        if (_key != EMPTY) {
            while (self.nodes[_key].left != EMPTY && self.nodes[_key].left >= _min) {
